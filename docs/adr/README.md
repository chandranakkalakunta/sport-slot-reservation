...
| [0043](ADR-0043-security-hardening.md) | Security Hardening | Accepted | Split by blast radius: PR-5a (low-risk — security-headers middleware, charter CORS/headers correction, CI container/dependency scanning, legacy containerregistry cleanup prepared, secret rotation policy) and PR-5b (risk-sensitive — Cloud Armor enforce, WIF least-privilege, deferred separately). Binary Authorization explicitly deferred to Phase 18 |
