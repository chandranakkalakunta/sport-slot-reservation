...
export default function CreateUser() {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setCreated({ email, temp_password: res.temp_password });
    } catch (e) {
  ...
  return (
    <>
      <main className="mx-auto max-w-lg px-4 py-10 space-y-6">
        ...
        <p className="text-sm">
          <Link to="/admin" className="block text-sm font-medium text-link underline underline-offset-2 hover:text-link/70">← Back to tenants</Link>
        </p>
      </main>

      {/* Credential modal — shown immediately on user creation; closing resets form for next user */}
      <TempPasswordModal
        creds={created ? [created] : null}
        title="User created"
        onClose={() => {
          setCreated(null);
