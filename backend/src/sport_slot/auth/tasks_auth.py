"""OIDC verification for Cloud Tasks -> worker endpoint calls (ADR-0019 §3).

Cloud Tasks signs each dispatch with a Google-minted OIDC ID token for the
configured invoker SA. We verify it the way any Google OIDC consumer would:
signature + issuer via google-auth, then pin audience and the calling SA's
email. No shared secret — the verified identity is the credential. Anything
else (missing token, bad signature, wrong audience, wrong SA) is rejected.
"""
...
def verify_tasks_oidc(request: Request, settings: Settings = Depends(get_settings)) -> None:
    if not settings.worker_base_url or not settings.tasks_invoker_sa:
        raise ApiError(500, error_codes.INTERNAL_ERROR, "Worker OIDC verification not configured")
